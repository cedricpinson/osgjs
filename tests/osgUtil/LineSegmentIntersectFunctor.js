    ...
    test('LineSegmentIntersectFunctor - check primitive type', function() {
        var checkPrimitive = function(geom, msg) {
            ...
            assert.isOk(
                ...
                msg + ' Intersections should be 1 and result is ' + lsi._intersections.length
            );
            var result = [0.4, 0.2, 0];
            var found = vec3.add(
                vec3.create(),
                start,
                vec3.scale(vec3.create(), dir, lsi._intersections[0]._ratio)
            );
            ...
            geom.accept(iv);
            assert.isOk(
                lsi2._intersections.length === 0,
                msg + ' Intersections should be 0 ' + lsi2._intersections.length
            );
        };
