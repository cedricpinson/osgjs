...
export default function() {
    ...
    var getChannelsInfo = function(animation) {
        ...
        for (var i = 0; i < nbChannels; ++i) {
            ...
            if (animation.duration === chan.end) nbChannelSyncEnd++;

            if (chan.times.buffer.byteLength - chan.times.byteLength === 4) nbTimesEmptySlot++;
        ...
        input
            ...
            .then(function(scene) {
                ...
                assert.approximately(
                    ...
                    1.93333 + 1.5,
                    1e-5,
                    'check duration of animation after lerpStartEnd'
                );
                ...
                assert.approximately(
                    ...
                    1.93333 + 1.5,
                    1e-5,
                    'check duration of animation after 2 calls to lerpStartEnd'
                );
                ...
                assert.isOk(info.nbChannelSyncEnd === 48, 'nb sync end channels');
                assert.isOk(
                    info.nbTimesEmptySlot === nbChannels - info.nbChannelSyncEnd,
                    'nb times empty slot'
                );
                assert.isOk(
                    info.nbKeysEmptySlot === nbChannels - info.nbChannelSyncEnd,
                    'nb times empty slot'
                );
