define( [
    ...
    return function () {
        ...
        test( 'Node', function () {
            ...
            ok( n._boundsComputed === false, 'boundingSphereComputed must be false' );
            n.getBound();
            ok( n._boundsComputed === true, 'boundingSphereComputed must be true' );

            var n1 = new Node();
            n.addChild( n1 );
            ok( n.children.length === 1, 'n must have 1 child' );
            ok( n1.parents.length === 1, 'n1 must have 1 parent' );
            ok( n._boundsComputed === false, 'boundingSphereComputed must be false after adding child' );
            n.getBound();
            ok( n._boundsComputed === true, 'boundingSphereComputed must be true after calling getBound' );


            n1.dirtyBound();
            ok( n._boundsComputed === false, 'boundingSphereComputed must be true if a child call dirtyBound' );
