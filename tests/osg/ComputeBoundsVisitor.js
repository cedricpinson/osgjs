...
module.exports = function () {
    ...
    !test( 'ComputeBoundsVisitor translate', function () {
        ...
        assert.equalVector( visitor.getBoundingBox().corner( 0, tmp ), vec3.fromValues( 7.5, -2.5, -2.5 ), 'Check Min of bounding box' );
        assert.equalVector( visitor.getBoundingBox().corner( 7, tmp ), vec3.fromValues( 12.5, 2.5, 2.5 ), 'Check Max of bounding box' );

        // getBoundingBox don't skip the 0 nodemask
        assert.equalVector( root.getBoundingBox().corner( 0, tmp ), vec3.fromValues( -12.5, -2.5, -2.5 ), 'Check Min of bounding box with getBoundingBox' );
        assert.equalVector( root.getBoundingBox().corner( 7, tmp ), vec3.fromValues( 12.5, 2.5, 2.5 ), 'Check Max of bounding box getBoundingBox' );
    ...
    test( 'ComputeBoundsVisitor translate and rotate', function () {
        ...
        assert.equalVector( visitor.getBoundingBox().corner( 0, tmp ), vec3.fromValues( -22.5, -2.5, -2.5 ), 'Check Min of bounding box' );
        assert.equalVector( visitor.getBoundingBox().corner( 7, tmp ), vec3.fromValues( -17.5, 2.5, 2.5 ), 'Check Max of bounding box' );

        // all the nodemask are active so same result as the visitor method
        assert.equalVector( root.getBoundingBox().corner( 0, tmp ), vec3.fromValues( -22.5, -2.5, -2.5 ), 'Check Min of bounding box with getBoundingBox' );
        assert.equalVector( root.getBoundingBox().corner( 7, tmp ), vec3.fromValues( -17.5, 2.5, 2.5 ), 'Check Max of bounding box getBoundingBox' );
