...
module.exports = function () {
    ...
    QUnit.test( 'BoundingSphere', function () {
        ...
        mockup.near( bscam.center(), Vec3.create() );
        ...
        main2.addChild( q3 );
        main2.addChild( mt3 );
        mockup.near( main2.getBound().center(), Vec3.create() );
    } );

    QUnit.test( 'BoundingSphere - expand with box', function () {
        ...
        mockup.near( bs.radius(), 3.4641, 1e-2, 'Expands with box - check radius' );
        mockup.near( bs.center(), Vec3.create(), 1e-2, 'Expands with box - check center' );

        Vec3.set( 2.0, 2.0, 2.0, bbox.getMin() );
        Vec3.set( 4.0, 4.0, 4.0, bbox.getMax() );
        bs.expandByBoundingBox( bbox );

        mockup.near( bs.radius(), 5.9135, 1e-2, 'Expands with box - check radius' );
        mockup.near( bs.center(), Vec3.createAndSet( 0.5857, 0.5857, 0.5857 ), 1e-2, 'Expands with box - check center' );
    } );
};
