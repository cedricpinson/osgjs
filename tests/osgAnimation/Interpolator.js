...
export default function() {
    test('Vec3LerpInterpolator', function() {
        ...
        assert.equalVector(
            result.value,
            [1.0, 1.0, 1.0],
            'Check value when time == 1.0 with 1 key'
        );
    ...
    test('QuatLerpChannel', function() {
        ...
        assert.equalVector(
            result.value,
            [1.22465e-16, 1.22465e-16, 1.22465e-16, -1],
            'Check value when time < first key'
        );
        assert.isOk(result.key === 0, 'Check key when time < first key');
        ...
        assert.equalVector(
            ...
            'Check value when time > last key'
        );
        assert.isOk(result.key === 0, 'Check key when time > last key');
