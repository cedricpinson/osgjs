...
export default function() {
    test('BasicAnimationManager', function() {
        ...
        assert.isOk(
            basicAnimationManager.getAnimations().AnimationTest !== undefined,
    ...
    test('BasicAnimationManager Linking', function() {
        ...
        assert.equal(
            animationCB.getName(),
            'testUpdateMatrixTransform',
            'check name of the first animation found'
        );

        assert.equal(
            basicAnimationManager._animationsUpdateCallbackArray.length,
            1,
            'check channel assigned to animation callback'
        );
        ...
        basicAnimationManager.update(null, nv);
        assert.equal(
            stackedRotateAxis.getTarget().value,
            0.5,
            'check target a value at t = ' + time
        );

        assert.equalVector(
            animationCallback._matrix,
            mat4.fromValues(
                0.8775825618903726,
                0.4794255386042031,
                0,
    ...
    test('BasicAnimationManager Controls', function() {
        ...
        assert.approximately(
            managerTime % duration,
            t,
            1e-5,
            'Value after set time factor at ' + timeFactor
        );
