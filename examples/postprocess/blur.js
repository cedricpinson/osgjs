    ...
    window.getPostSceneBlur = function(/* sceneTexture*/) {
        ...
        getSceneTexture = function(sceneFile, image) {
            if (cachedScenes[sceneFile] === undefined) {
                if (image) cachedScenes[sceneFile] = osg.Texture.createFromImage(image);
                else cachedScenes[sceneFile] = osg.Texture.createFromURL(sceneFile);
            }
...
*/
                inputTex = new osgUtil.Composer.Filter.InputTexture(
                    getSceneTexture('Budapest.jpg')
                );
