...
var main = function() {
  ...
  // The 3D canvas.
    ...
    function subTileLevelRowCol(subTileId, level, row, col )
    {
      ...
      if(subTileId===1) { x = 1;}
      else if(subTileId === 2) { y = 1; }
      else if(subTileId === 3) { x = 1; y = 1; }
      
      sLevel = level+1;
      sCol = col*2+x;
      sRow = row*2+y;
      return {sLevel : sLevel, sCol : sCol, sRow : sRow };
    }
    ...
    function levelRowColToXYWidthHeight(rootLevel, level, row, col)
    {
      ...
      return {x : tileExtent.minx , y : tileExtent.miny, width : width, height : height};
