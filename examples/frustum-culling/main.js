    ...
    FrustumCullingDebugCallback.prototype = {
        update: function(node, nv) {
            ...
            nv.getNodePath().forEach(function(a) {
                ...
                if (
                    a.getName() === 'debugSphere' ||
                    (a.getParents()[0] && a.getParents()[0].getName() === 'debugSphere')
                ) {
                    debugSphere = true;
                }
            });
...
*/
                            ...
                            transformSphere.setMatrix(
                                osg.mat4.fromTranslation(osg.mat4.create(), [
                                    2.5 + wG * groundSize - 100,
                                    2.5 + wH * groundSize - 100,
                                    -5.0
                                ])
                            );
        ...
        addFrameBufferView: function() {
            ...
            this._ComposerdebugCamera.setViewport(
                new osg.Viewport(0, 0, this._canvas.width, this._canvas.height)
            );
            ...
            this._ComposerdebugCamera.setClearMask(
                !(osg.Camera.COLOR_BUFFER_BIT | osg.Camera.DEPTH_BUFFER_BIT)
            );
            var texture;
            ...
            if (!optionsDebug.fullscreen) stateset.setAttributeAndModes(new osg.Depth('ALWAYS'));
        ...
        createScene: function() {
            ...
            this._viewer
                .getCamera()
                .getRenderer()
                .setCullVisitor(new CustomCullVisitor(this._config));
