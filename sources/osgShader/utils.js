...
var callFunction = function(funcName, output, inputs) {
    ...
    if (output) {
        if (output.getVariable) output = output.getVariable();
        callString = output + ' = ';
    }
    ...
    if (inputs && inputs.length > 0) {
        ...
        for (var i = 0, l = inputs.length; i < l; i++) {
            ...
            if (inputs[i].getVariable) callString += inputs[i].getVariable();
            else callString += inputs[i];
    ...
    if (osgShader.debugShaderNode) return debug.join('\n') + '\n' + callString;
