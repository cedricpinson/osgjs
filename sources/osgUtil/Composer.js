    ...
    MACROUTILS.objectInherit(Node.prototype, {
        ...
        debugCheckRttNotReadWrite: function(textureResult, stateSet) {
            for (var k = 0, lt = stateSet.getNumTextureAttributeLists(); k < lt; k++) {
                ...
                Notify.assert(
                    textureResult !== textureBinded,
                    'Composer: write/read at the same time on a texture is undefined behavior'
                );
            }
        },
        /*develblock:end*/
            ...
            this._stack.forEach(function(element, i, array) {
                ...
                if (element.texture !== undefined) {
                    ...
                    if (inputTexture === undefined) {
                        Notify.warn(
                            "Composer can't find any information to setup texture output size"
                        );
                    } else {
                ...
                var lastFilterRenderToScreen =
                    i === array.length - 1 && self._renderToScreen === true;
                    ...
                    if (stateSet.getAttribute('Program') === undefined) {
                        array[i + 1].filter
                            ...
                            .setTextureAttributeAndModes(
                                0,
                                stateSet.getTextureAttribute(0, 'Texture')
                            );
                        return;
                    }
                }
                ...
                if (lastFilterRenderToScreen === true) {
                    ...
                    camera.attachTexture(
                        FrameBufferObject.COLOR_ATTACHMENT0,
                        textureResult,
                        textureTarget
                    );
                }
                ...
                if (element.filter.buildGeometry) quad = element.filter.buildGeometry(quad);
                ...
                if (textureResult) {
                    ...
                    if (i + 1 < array.length) {
                        array[i + 1].filter
                            .getStateSet()
                            .setTextureAttributeAndModes(0, textureResult);
                    }
                }
                ...
                element.filter
                    .getStateSet()
                    .addUniform(Uniform.createFloat2(vec2.fromValues(w, h), 'RenderSize'));
...
Composer.Filter.Helper = {
    ...
    getOrCreatePascalCoefficients: function(kernelSize) {
        ...
        if (cache[kernelSize]) return cache[kernelSize];
        for (var j = cache.length - 1; j < kernelSize; j++) {
            ...
            // normalized array
            sum = Math.pow(2, j + 1);
            for (var k = 0; k < currentRowSize + 1; k++) nextRow[k] /= sum;
            cache.push(nextRow);
        }
        ...
        build: function() {
            this._program = new Program(
                ...
                new Shader(
                    ...
                    this._fragmentShader + this.getDefineFragmentName()
                )
            );

            if (this._uniforms) {
                this.autoBindFragmentUniformStateSet(
                    this._stateSet,
...
MACROUTILS.createPrototypeObject(
    Composer.Filter.PingPong,
    MACROUTILS.objectInherit(Composer.Filter.Custom.prototype, {
        // Constraints:
        ...
        interConnectFilters: function(composer, i, array) {
            ...
            var unifKey,
                unif,
                uniforms = this.getStateSet().getUniformList();
            if (uniforms) {
            ...
            if (this.buildGeometry) quad = this.buildGeometry(quad);
    ...
    MACROUTILS.objectInherit(Composer.Filter.prototype, {
        ...
        build: function() {
            ...
            var fgt = [
                ...
                this._unpack ||
                    'vec4 unpack(const in sampler2D tex, const in vec2 uv) { return texture2D(tex, uv); }',
...
MACROUTILS.createPrototypeObject(
    Composer.Filter.AverageVBlur,
    MACROUTILS.objectInherit(Composer.Filter.AverageHBlur.prototype, {
        getUVOffset: function(value) {
            return 'vec2(0.0, float(' + value + ')/RenderSize[1]);';
        }
    }),
    'osgUtil',
    'ComposerFilterAverageVBlur'
);
...
MACROUTILS.createPrototypeObject(
    Composer.Filter.BilateralVBlur,
    MACROUTILS.objectInherit(Composer.Filter.BilateralHBlur.prototype, {
        getUVOffset: function(value) {
            return 'vec2(float(' + value + ')*pixelSize/RenderSize[0],0.0);';
        }
    }),
    'osgUtil',
    'ComposerFilterBilateralVBlur'
);
...
MACROUTILS.createPrototypeObject(
    ...
    MACROUTILS.objectInherit(Composer.Filter.prototype, {
        ...
        build: function() {
            ...
            var kernel = [];
            kernel.push(
                ' pixel = float(' + weights[start] + ')*unpack(Texture0, vTexCoord0 ).rgb;'
            );
            ...
            if (!this._linear) {
                ...
                for (i = start + 1; i < nbSamples; i++) {
                    ...
                    kernel.push(
                        ' pixel += ' +
                            weight +
                            '* unpack(Texture0, (vTexCoord0.xy + offset.xy)).rgb;'
                    );
                    kernel.push(
                        ' pixel += ' +
                            weight +
...
MACROUTILS.createPrototypeObject(
    Composer.Filter.VBlur,
    MACROUTILS.objectInherit(Composer.Filter.HBlur.prototype, {
        getUVOffset: function(value) {
            return 'vec2(0.0, float(' + value + ')/RenderSize[1]) ;';
        }
    }),
    ...
    MACROUTILS.objectInherit(Composer.Filter.prototype, {
        ...
        build: function() {
            ...
            var fgt = [
                ...
                '  float magSqr = dot(rowy,rowy) + dot(rowx,rowx);',
                '  bool infMag = magSqr < 1.0/65025;',
                '  vec3 texelCenter = texture2D(Texture0, vTexCoord0).rgb;',
                '  if (texelCenter == vec3(0.0) || (uHighlightFactor == 0.0 && infmag)) discard;',
                '  float finalFactor = infMag ? uHighlightFactor : min(1.0, sqrt(magSqr) * uOutlineFactor);',
                '  gl_FragColor = finalFactor * vec4(texelCenter, 1.0);',
                '}',
                ''
            ].join('\n');
    ...
    MACROUTILS.objectInherit(Composer.Filter.prototype, {
        ...
        build: function() {
            ...
            if (uniform === undefined) {
                uniform = Uniform.createFloat2(
                    vec2.fromValues(
                        this._size[0] / this._noiseTextureSize,
                        this._size[1] / this._noiseTextureSize
                    ),
                    'noiseSampling'
                );
                stateSet.addUniform(uniform);
...
MACROUTILS.createPrototypeObject(
    Composer.Filter.SSAO8,
    MACROUTILS.objectInherit(Composer.Filter.SSAO.prototype, {
