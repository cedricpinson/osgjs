    ...
    DisplayNodeGraphVisitor.prototype = MACROUTILS.objectInherit( NodeVisitor.prototype, {
        ...
        apply: function ( node ) {
            ...
            if ( this._fullNodeList[ node.getInstanceID() ] !== node ) {
                ...
                var stateset = '';
                var ns = node.getStateSet();
                if ( ns ) {
                    var stateID = ns.getInstanceID();
                    stateset = this._fullNodeList[ stateID ];
                    if ( !stateset ) {
                        stateset = this.createStateset( node, stateset );
                        this._fullNodeList[ stateID ] = ns;
                    }
                }
        ...
        generateNodeAndLink: function ( g ) {
            ...
            for ( var i = 0; i < nodeLength; i++ ) {
                ...
                g.addNode( element.instanceID, {
                    ...
                    description: ( element.stateset && element.matrix !== '' ? '<br /><br />' : '' ) + element.matrix
                } );
