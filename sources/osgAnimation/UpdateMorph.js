define( [
    ...
    UpdateMorph.prototype = MACROUTILS.objectInherit( AnimationUpdateCallback.prototype, {
        ...
        _computeExtraWeightsSum: function () {
            ...
            // check comment in _morphBufferArrayCPU (we have to avoid near zero value)
            var eps = 1e-5;
        ...
        _morphBufferArrayCPU: function () {
            ...
            // compute new weights of the 4th target (all the extra target will be merged inside this one)
            var extraWeightSum = this._weights[ 3 ] = this._computeExtraWeightsSum();

            var processed = {}; // handles referenced buffer array (avoid useless twice morph computation on same array)
            var morphs = this._morphs;
            for ( var i = 0, nbMorphs = morphs.length; i < nbMorphs; ++i ) {
                ...
                for ( var j = 0, nbNames = morphNames.length; j < nbNames; ++j ) {
                    ...
                    // skip if the bufferArray is shaded in another morphGeometry and has already been cpu morphed
